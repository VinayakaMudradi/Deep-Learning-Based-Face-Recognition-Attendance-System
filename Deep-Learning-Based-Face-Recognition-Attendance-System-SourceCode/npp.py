...
for i in range(0,len(imageData)):
    ...
    print(predictions) 
    for i in range(number_of_students[0][0]):
        print(predictions[0][i])
        if predictions[0][i]>0.90:
            print(predictions[0][i])
            person_found=1
            Query="SELECT * FROM student WHERE ID='%d'" %(int(i+1))
            ...
            print(person_name[i])
            print("\n")
            sheet.cell(row=int(i+2),column=max_col_number+1,value="Present").font=Font(color='228B22')
